...
export default function CalendarSection({ modelId }: CalendarSectionProps) {
  ...
  const eventStyleGetter = (event: any) => {
    ...
    // Status colors: Red=Not Started, Yellow=In Progress, Green=Done
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {showAddModal && (
        <CalendarEventModal
          event={selectedEvent || { start: new Date(), end: new Date(), title: '', notes: '', status: 'not_started', isBlocked: false, isOnline: false }}
          onClose={() => {
            setShowAddModal(false);
            setSelectedEvent(null);
          }}
